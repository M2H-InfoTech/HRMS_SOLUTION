...
{
    public class ShiftSettingsRepository : IShiftSettingsRepository
    {
        private readonly AttendanceDbContext _context;
        private readonly EmployeeDBContext _employeeDbContext;

        // Normal constructor for DI
        public ShiftSettingsRepository(AttendanceDbContext attendanceDbContext, EmployeeDBContext employeeDBContext)
        {
            _context = attendanceDbContext;
        ...
        {
            var shiftPeriods = await _context.HrShift00s
        ...
        public async Task<PaginatedResultDto> GetShiftAccessDetails(int shiftAccessId, int entryBy, int roleId, int status, int empStatus, int pageNumber, int pageSize)
        {
            ...
            if (lnkLev == 15)
            {
                ...
                var accessEntities = await _context.EntityAccessRights02s
                ...
                var empEntityList = (await _context.HrEmpMasters
            ...
            else
            {
                ...
                var empEntityString =await _context.HrEmpMasters
                    .Where(e => e.EmpId == newEmpId)
                ...
                var data = await _context.HrEmpMasters.ToListAsync();
                var hrmReportings = await _context.HrEmpReportings.ToListAsync();

                var highLevelViews = await _context.HighLevelViewTables.ToListAsync();
                ...
                var employeeIds = _context.EmployeeDetails
                    .Where(a =>
        ...
        {
            return await _employeeDbContext.HrmValueTypes.ToListAsync();
